...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>ANDROID HUB chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "ANDROID HUB", HidePremium = false, IntroText = "ANDROID HUB⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("ANDROID HUB SCRIPT")
W:AddLabel("DiscordANDROID HUB")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "ANDROID HUB(Noitification)",
